"use client";

import { useState, useEffect, useCallback } from "react";
import AdminShell from "@/components/admin/AdminShell";
import { Trash2, Plus, Loader2 } from "lucide-react";

interface CmsBoardResult {
  id: string;
  title: string;
  date: string;
  meetingNumber: number;
  meetingType: "정기" | "임시";
  agenda: string[];
  content: string;
  createdAt: string;
}

function BoardManager({ password }: { password: string }) {
  const [results, setResults] = useState<CmsBoardResult[]>([]);
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [msg, setMsg] = useState("");

  const [title, setTitle] = useState("");
  const [date, setDate] = useState(new Date().toISOString().slice(0, 10));
  const [meetingNumber, setMeetingNumber] = useState(1);
  const [meetingType, setMeetingType] = useState<"정기" | "임시">("정기");
  const [agenda, setAgenda] = useState("");
  const [content, setContent] = useState("");

  const fetchResults = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/board", {
        headers: { "x-admin-password": password },
      });
      if (res.ok) {
        const data = await res.json();
        setResults(data.results ?? []);
      }
    } finally {
      setLoading(false);
    }
  }, [password]);

  useEffect(() => {
    fetchResults();
  }, [fetchResults]);

  const handleSubmit = async (e: { preventDefault(): void }) => {
    e.preventDefault();
    setSubmitting(true);
    setMsg("");
    try {
      const res = await fetch("/api/admin/board", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-admin-password": password,
        },
        body: JSON.stringify({ title, date, meetingNumber, meetingType, agenda, content }),
      });
      if (res.ok) {
        setMsg("✓ 이사회 결과가 등록되었습니다.");
        setTitle("");
        setDate(new Date().toISOString().slice(0, 10));
        setMeetingNumber((prev) => prev + 1);
        setAgenda("");
        setContent("");
        fetchResults();
      } else {
        const err = await res.json();
        setMsg(`오류: ${err.error}`);
      }
    } catch {
      setMsg("서버 오류가 발생했습니다.");
    }
    setSubmitting(false);
  };

  const handleDelete = async (id: string) => {
    if (!confirm("이 항목을 삭제하시겠습니까?")) return;
    const res = await fetch(`/api/admin/board/${id}`, {
      method: "DELETE",
      headers: { "x-admin-password": password },
    });
    if (res.ok) fetchResults();
  };

  const formatDate = (d: string) => d.replace(/-/g, ".");

  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 space-y-8">
      <div>
        <p className="text-[11px] font-bold uppercase tracking-[0.25em] text-gold mb-1">Admin</p>
        <h1 className="text-2xl font-bold text-navy-900">이사회 결과 관리</h1>
        <p className="text-[13px] text-navy-500 mt-1">이사회 개최 결과를 등록하거나 삭제할 수 있습니다.</p>
      </div>

      {/* 등록 폼 */}
      <div className="bg-white rounded-2xl border border-navy-100 shadow-sm p-6 sm:p-8">
        <h2 className="text-[15px] font-bold text-navy-900 mb-5 flex items-center gap-2">
          <Plus className="h-4 w-4 text-gold" />
          새 이사회 결과 등록
        </h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid sm:grid-cols-3 gap-4">
            <div className="sm:col-span-2">
              <label className="block text-[12px] font-semibold text-navy-600 mb-1.5">제목 *</label>
              <input
                type="text"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder="예: 제1회 정기 이사회 결과"
                required
                className="w-full rounded-xl border border-navy-200 px-4 py-2.5 text-[14px] text-navy-900 placeholder:text-navy-300 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-all"
              />
            </div>
            <div>
              <label className="block text-[12px] font-semibold text-navy-600 mb-1.5">날짜 *</label>
              <input
                type="date"
                value={date}
                onChange={(e) => setDate(e.target.value)}
                required
                className="w-full rounded-xl border border-navy-200 px-4 py-2.5 text-[14px] text-navy-900 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-all"
              />
            </div>
          </div>

          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-[12px] font-semibold text-navy-600 mb-1.5">회차</label>
              <div className="flex items-center gap-2">
                <span className="text-[14px] text-navy-600">제</span>
                <input
                  type="number"
                  value={meetingNumber}
                  onChange={(e) => setMeetingNumber(Number(e.target.value))}
                  min={1}
                  className="w-20 rounded-xl border border-navy-200 px-4 py-2.5 text-[14px] text-navy-900 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-all text-center"
                />
                <span className="text-[14px] text-navy-600">회</span>
              </div>
            </div>
            <div>
              <label className="block text-[12px] font-semibold text-navy-600 mb-1.5">종류</label>
              <select
                value={meetingType}
                onChange={(e) => setMeetingType(e.target.value as "정기" | "임시")}
                className="w-full rounded-xl border border-navy-200 px-4 py-2.5 text-[14px] text-navy-900 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-all bg-white"
              >
                <option value="정기">정기 이사회</option>
                <option value="임시">임시 이사회</option>
              </select>
            </div>
          </div>

          <div>
            <label className="block text-[12px] font-semibold text-navy-600 mb-1.5">
              의결 안건{" "}
              <span className="text-navy-400 font-normal">(한 줄에 하나씩 입력)</span>
            </label>
            <textarea
              value={agenda}
              onChange={(e) => setAgenda(e.target.value)}
              placeholder={"제1호 안건: 재단 정관 승인\n제2호 안건: 2026년도 예산안 심의"}
              rows={4}
              className="w-full rounded-xl border border-navy-200 px-4 py-3 text-[14px] text-navy-900 placeholder:text-navy-300 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-all resize-y"
            />
          </div>

          <div>
            <label className="block text-[12px] font-semibold text-navy-600 mb-1.5">
              회의 결과 상세
            </label>
            <textarea
              value={content}
              onChange={(e) => setContent(e.target.value)}
              placeholder="이사회 개최 결과 및 주요 의결 사항을 상세히 입력하세요."
              rows={6}
              className="w-full rounded-xl border border-navy-200 px-4 py-3 text-[14px] text-navy-900 placeholder:text-navy-300 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-all resize-y"
            />
          </div>

          <div className="flex items-center justify-between">
            {msg && (
              <p className={`text-[13px] ${msg.startsWith("✓") ? "text-green-600" : "text-red-600"}`}>
                {msg}
              </p>
            )}
            <div className="ml-auto">
              <button
                type="submit"
                disabled={submitting}
                className="flex items-center gap-2 rounded-xl bg-navy-900 text-white text-[14px] font-semibold px-6 py-2.5 hover:bg-navy-700 disabled:opacity-50 transition-colors"
              >
                {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}
                등록
              </button>
            </div>
          </div>
        </form>
      </div>

      {/* 목록 */}
      <div className="bg-white rounded-2xl border border-navy-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-navy-100 flex items-center justify-between">
          <h2 className="text-[15px] font-bold text-navy-900">등록된 이사회 결과</h2>
          <span className="text-[13px] text-navy-400">총 {results.length}건</span>
        </div>

        {loading ? (
          <div className="py-16 flex justify-center">
            <Loader2 className="h-6 w-6 animate-spin text-navy-300" />
          </div>
        ) : results.length === 0 ? (
          <div className="py-16 text-center text-[14px] text-navy-400">
            등록된 이사회 결과가 없습니다.
          </div>
        ) : (
          <ul className="divide-y divide-navy-50">
            {results.map((result) => (
              <li
                key={result.id}
                className="flex items-start gap-4 px-6 py-4 hover:bg-navy-50/50 transition-colors"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-navy-100 text-navy-600 border border-navy-200">
                      제{result.meetingNumber}회 {result.meetingType}
                    </span>
                  </div>
                  <p className="text-[14px] font-medium text-navy-800 line-clamp-1">{result.title}</p>
                  <p className="text-[12px] text-navy-400 mt-0.5">{formatDate(result.date)}</p>
                  {result.agenda.length > 0 && (
                    <p className="text-[12px] text-navy-400 mt-1">
                      안건 {result.agenda.length}건
                    </p>
                  )}
                </div>
                <button
                  onClick={() => handleDelete(result.id)}
                  className="flex-shrink-0 flex items-center gap-1.5 text-[12px] text-navy-400 hover:text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors"
                >
                  <Trash2 className="h-3.5 w-3.5" />
                  삭제
                </button>
              </li>
            ))}
          </ul>
        )}
      </div>
    </div>
  );
}

export default function AdminBoardPage() {
  return (
    <AdminShell>
      {(password) => <BoardManager password={password} />}
    </AdminShell>
  );
}
